...
def restate3D(arrPol:np.ndarray,arrMol:np.ndarray,
              work_title:str = "Computing")->np.ndarray:
    """
...
def progressbar(step, length, time_elapsed,prefix = "Computing")->None:
    ...
    print(f'{prefix}: [{u"█"*step_norm}{" "*remaining_norm}]{step}/{length} -- Step/s:{per:.1f} -- ETA:{eta:.1f}{unit}     '
          ,flush=True,end=ending)
