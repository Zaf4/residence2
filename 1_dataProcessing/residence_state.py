...
def restate2D(arr1: np.ndarray, arr2: np.ndarray) -> np.ndarray:
    """
    ...
    -------
    state : np.ndarray
        bound states of molecules to ploymer.
...
def restate3D(
    arrPol: np.ndarray, arrMol: np.ndarray, work_title: str = "Computing"
) -> np.ndarray:
    """
    ...
    work_title : str, optional
        Title for the job. The default is "Computing".
    ...
    # creates a 3D state array to contain state for each bead at each time
    state_whole = np.zeros([time_range, mol_num], dtype=np.float32)
...
def progressbar(
    step: int, length: int, time_elapsed: float, prefix: str = "Computing"
) -> None:
    ...
    print(
        f"{prefix}: [{'█' * step_norm}{' ' * remaining_norm}]{step}/{length} -- Step/s:{per:.1f} -- ETA:{eta:.1f}{unit}     ",
        flush=True,
        end=ending,
    )
...
def state_all(fname: os.PathLike = "dump.npy", ignore: int = 1001) -> None:
    ...
    ignore : int, optional
        Number of timestep to ignore. The default is 1001.

    Returns
    -------
    None
        This function saves the files and returns nothing
