...
def txt2arr(ftoread: os.PathLike = "dump.main", ignore: int = 0) -> np.ndarray:
    """
    ...
    Parameters
    ----------
    ftoread : os.PathLike, optional
        path/to/dumpfile. The default is "dump.main".
    ignore : int, optional
        Number of initial timesteps to ignore. The default is 0.

    Returns
    -------
    arr : np.ndarray
        dump array converted from dumpfile.
    ...
    with open("summary.txt", "w") as summary:
        ...
        summary.write(
            f"Compression level: \t{100 - arr.size * arr.itemsize / size * 100:.2f}%\n"
        )
