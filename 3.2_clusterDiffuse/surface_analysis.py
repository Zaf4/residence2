...
for i,df in enumerate(dfs):
    ...
    sns.barplot(data=df,
                  ...
                  errcolor='k', errwidth=2, capsize=0.2)
    

    #Swarmplot-----------------------------------------------------------------
    swarm_data = df.sample(frac=0.1,random_state = 42)
    sns.swarmplot(data=swarm_data,
                  x='kind',y='residence',
                  ax=axes[1,i],size=4,order = ['free','surf','core'],
    ...
    for j in range(2):
        ...
        # ax.set_ylim([lower,round_up(upper)])

sns.despine(trim=True)
plt.tight_layout()
plt.savefig('./graphs/plot.pdf')
