...
@_modify_scatter
def system_scatter(coor: pd.DataFrame, **kwargs) -> plt.Axes:
    ...
    ax = sns.scatterplot(
        data=capsid, **kwargs, palette="mako", edgecolor="k", linewidth=0.6
    )

    plt.text(x=-90, y=27, s=f"Rg={Rg(coor):.1f}", fontdict=font)

    return ax


@_modify_scatter
def protein_scatter(coor: pd.DataFrame, **kwargs) -> plt.Axes:
...
@_modify_scatter
def cluster_scatter(coor: pd.DataFrame, **kwargs) -> plt.Axes:
    ...
    ax = sns.scatterplot(
        data=capsid, **kwargs, palette="mako", edgecolor="k", linewidth=0.6, hue="type"
    )
...
def createFigures(coor: pd.DataFrame, case):

    if os.path.exists("./figures") == False:
        os.mkdir("./figures")

    # entire system
    entire = system_scatter(coor, x="x", y="y", hue="type")
    plt.savefig(
        f"./figures/overview{case}.png", dpi=400, transparent=1, bbox_inches="tight"
    )
    # proteins alone
    proteins = protein_scatter(coor, x="x", y="y", hue="type")
    plt.savefig(
        f"./figures/proteins{case}.png", dpi=400, transparent=1, bbox_inches="tight"
    )
    # only clusters
    clusters = cluster_scatter(coor, x="x", y="y")
    plt.savefig(
        f"./figures/clusters{case}.png", dpi=400, transparent=1, bbox_inches="tight"
    )
