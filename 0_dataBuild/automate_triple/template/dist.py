...
def read_residence(resident):
        ...
        for res in resident:  # for each row or the binding molecule
            ...
            for r in res:  # binding status 0 or 1,2
                if r:
                    ...
                    r == 0
                ):  # when the binding status is not satisfied anymore, binding duration is saved and time reset to 0
    ...
    )  # empty array to store durations vs occurence rates
    distribution[:, 0] = serial  # first row is durations

    for i in range(length):  # counting occurences
        distribution[i, 1] = np.count_nonzero(
            bound == distribution[i, 0]
        )  # how many occurences for occurences observed for each duration
