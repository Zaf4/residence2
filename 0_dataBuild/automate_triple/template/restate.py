...
def txt2arr(ftoread="dump.main", ignore=1001):
    ...
    for i in range(ignore_num, time_num):
        ...
        if i % (int(time_interval / 100)) == 0 or i == time_num - 1:
            progressbar(
                i - ignore_num + 1, time_interval, tof - tos, prefix="Text to array"
            )
...
def restate3D(arrPol, arrMol, work_title="Computing"):
    ...
    state_whole = np.zeros(
        [time_range, mol_num], dtype=np.float32
...
if __name__ == "__main__":
    ...
    fl_nap = np.add(
        lfoot, rfoot
    )  # partials (foots) are added together 1 means partial 2 means complete binding
    sp_nap = np.add(
        sp_lfoot, sp_rfoot
    )  # only sp interactions, 0:free, 1: one foot only, 2: both feet
    ns_nap = np.add(
        ns_lfoot, ns_rfoot
    )  # only ns interactions, 0:free, 1: one foot only, 2: both feet
