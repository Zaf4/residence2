...
def addClusterLifetimes(
    df: pd.DataFrame, states: np.ndarray, timestep: int = 1000, duration: int = 2000
) -> pd.DataFrame:
...
def addResidenceTimes(
    df: pd.DataFrame, states: np.ndarray, timestep: int = 1000
) -> pd.DataFrame:
...
def addSurface(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in np.arange(num_clust) + 1:
        ...
        atomIDs = np.array(
            partial.atomID
        )  # storing indexes before removing binding domains
        # marking core atoms
...
def generateDF(
    dumpfile: os.PathLike = "./data/targetDUMP.npy",
...
def makePlot(df: pd.DataFrame, *args, **kwargs) -> plt.Axes:
    ...
    fig, ax = plt.subplots(
        1, 2, figsize=(16, 4.5), gridspec_kw={"width_ratios": [2.5, 1]}
    )
    # showing the system -------------SCATTER-----------------------------------
    sns.scatterplot(
        df[df.timestep == max_time],
        x="x",
        y="y",
        hue="tau",
        palette="Purples",
    ...
    ax[0].figure.colorbar(
        purples, ax=ax[0], location="right", shrink=1, label="Mean Lifetime"
    )
    ax[0].figure.tick_params(labelsize=30)

    # regression  -----------------REGRESSION-----------------------------------
    sns.regplot(df, x="clusterSize", y="tau", ax=ax[1], color="red")
    ...
    ax[1].annotate(
        xy=(0.85, 0.05), text=f"N = {N}", fontweight="bold", xycoords="axes fraction"
    )
...
def makePlotMulti_tau(dfs: list, *args, **kwargs) -> plt.Axes:
    ...
    num_rows = len(dfs)
    fig, ax = plt.subplots(
        num_rows, 2, figsize=(20, 4 * num_rows), gridspec_kw={"width_ratios": [1.2, 1]}
    )
    ...
    for row, df in enumerate(dfs):
        ...
        # plotting the residence by TFs -------------SCATTER-----------------------------------
        sns.scatterplot(
            data=df[df.timestep == time],
            x="x",
            y="y",
            hue="residence_log",
            palette="Purples",
        ...
        # plotting the surface-Kind -------------SCATTER-----------------------------------
        sns.scatterplot(
            data=df[df.timestep == time],
            x="x",
            y="y",
        ...
        ax[row, 1].set_ylabel("")
        ax[row, 1].set_xlabel("")
        ax[row, 1].set_xlim([-92, 92])
        ax[row, 1].set_ylim([-31, 31])
        ax[row, 1].set_xticks([])
        ax[row, 1].set_yticks([])
        # adding color bar-----------------------------------------------------------
        norm = plt.Normalize(df.residence_log.min(), df.residence_log.max())
        purples = plt.cm.ScalarMappable(cmap="Purples", norm=norm)
        purples.set_array([])
        ax[row, 1].get_legend().remove()
        ax[row, 0].get_legend().remove()

        ax[row, 0].figure.colorbar(
            purples,
            ax=ax[row, 0],
            location="left",
            shrink=0.9,
        ...
        sns.despine(ax=ax[row, 1], left=True, bottom=True)
        # regression  -----------------REGRESSION-----------------------------------
...
if __name__ == "__main__":
    ...
    if "celestia" in platform.node():  # run this part only if pc name includes celestia
        ...
        df280 = multiTimeDF(
            ...
            statesfile=statesfiles[0],
            kT=float(kts[0]),
        )
        df300 = multiTimeDF(
            timesteps,
            dumpfile=dumpfiles[1],
            statesfile=statesfiles[1],
            kT=float(kts[1]),
        )
        df350 = multiTimeDF(
            timesteps,
            dumpfile=dumpfiles[2],
            statesfile=statesfiles[2],
            kT=float(kts[2]),
        )
        df400 = multiTimeDF(
            timesteps,
            dumpfile=dumpfiles[3],
            statesfile=statesfiles[3],
            kT=float(kts[3]),
        )
        ...
        mapping = {0: "free", 1: "surf", 2: "core"}
        for df in dfs:
            df["kind"] = df["kind"].replace(mapping)
            df["residence_log"] = np.log2(np.array(df["residence"]) + 2)
            print(df.sample(10).residence)
