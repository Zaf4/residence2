...
def imagify(frame:pd.DataFrame,palette:str='viridis')->plt.Axes:
    ...
    sns.scatterplot(data=frame[frame.clusterID>0], 
                    x = 'x',y = 'y',
                    hue='clusterID',
                    palette=palette,
                    legend=None)
...
def frame_clusters2current(arr_name:str,skip:int=1)->None:
    ...
    matplotlib.use('agg')
    for i,frame in track(enumerate(frames),total=len(frames),
...
def images2movie(movie_name:str='clusters',
                 path:os.PathLike='./images')->None:
    """
...
def generate_cluster_info_table(frame:np.ndarray)->pd.DataFrame:
    """
...
def labeledGraph(arr_name:str,index:int=-1,savepng:bool=False)->plt.Axes:
    ...
    ss = cluster_single_frame(frame)
    ax = imagify(ss)
    table,matrix = generate_cluster_info_table(frame)
...
if __name__ == '__main__':
    ...
    dump = np.load('./sampleDumps/small_280_60.npy')
    single_frame = cluster_single_frame(dump[-120])
    cluster_frame=mark_cluster_surface(cluster_single_frame(dump[-120]))
    info_table,_ = generate_cluster_info_table(dump[-120])
    ax =labeledGraph('./sampleDumps/small_280_60.npy',index=-300)
