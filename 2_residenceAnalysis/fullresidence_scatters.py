...
def scatterit_multi(
    df: pd.DataFrame, fits: pd.DataFrame, i: int, j: int, axes, palette: str, **kwargs
) -> None:
    ...
    for i, eqname in track(enumerate(eqnames), total=len(eqnames)):
        for j, keyword in enumerate(keywords):
            ...
            if "." in keyword:  # if for energy
                scatterit_multi(
                    partial_data, partial_fits, axes=axes, palette="mako_r", i=i, j=j
                )
...
if __name__ == "__main__":
    ...
    fig = generate_fit_graph(keywords=ums, figname="FORMER-fig2A")  # fig2
