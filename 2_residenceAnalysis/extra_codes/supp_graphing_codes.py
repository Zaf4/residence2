...
def residuals2boxplot(residues: pd.DataFrame, cpalette="flare"):
    ...
    plt.ylim([1e-1, 1e5])
    plt.savefig(
        "./scatters/eqVSresidues.png", dpi=400, transparent=True, bbox_inches="tight"
    )
...
def scatterit_manual(df: pd.DataFrame, fits: pd.DataFrame, ind: int = 0) -> plt.Axes:
    ...
    # scatter plots
    sns.scatterplot(
        data=df[vlo], color=paor[0], s=50, edgecolor="black", linewidth=0.25
    )
    sns.scatterplot(
        data=df[low], color=paor[1], s=50, edgecolor="black", linewidth=0.25
    )
    sns.scatterplot(
        data=df[mid], color=paor[2], s=50, edgecolor="black", linewidth=0.25
    )
...
def scatterit_novel_palette(
    df: pd.DataFrame, fits: pd.DataFrame, colors: list = ["#2456E5", "#C70039"]
) -> plt.Axes:
    """
    ...
    for i, col in enumerate(df):
        # scatterplot
        sns.scatterplot(
            data=df[col], color=deeper[i], s=50, edgecolor="black", linewidth=0.25
        )
    for i, col in enumerate(fits):
        # lineplot
        sns.lineplot(data=fits[col], color=lighter[i], linestyle="dashed", alpha=1)
...
def generate_fit_graph(datafile: str, fitfile: str, keyword: str) -> None:
    ...
    if "." in keyword:  # if for energy
        ax = scatterit_novel_palette(
            partial_data, partial_fits, colors=["#D6DBDF", "#1C2833"]
        )
        plt.text(x=1, y=1, color="grey", s=f"Fit equation:{fname}\nEnergy:{keyword}kT")
        legend = [f"{x[-2:]}µM" for x in cols]

    else:
        ax = scatterit_novel_palette(partial_data, partial_fits)
        plt.text(
            x=1, y=1, color="grey", s=f"Fit equation:{fname}\nConcentration:{keyword}µM"
        )
        legend = [f"{x[:4]}kT" for x in cols]

    plt.legend(legend)
    plt.savefig(
        f"./scatters/{fname}_{keyword}.png",
        dpi=400,
        transparent=False,
        bbox_inches="tight",
    )
