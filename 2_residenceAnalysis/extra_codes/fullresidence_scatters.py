...
@Rplotify
def scatterit(
    df: pd.DataFrame, fits: pd.DataFrame, palette: str, residuals: bool = True
) -> plt.Axes:
    """
    ...
    for i, col in enumerate(df):
        # scatterplot
        sns.scatterplot(data=df[col], s=50, edgecolor="black", linewidth=0.25)
    for i, col in enumerate(fits):
        # lineplot
        sns.lineplot(data=fits[col], linestyle="dashed", alpha=1)
