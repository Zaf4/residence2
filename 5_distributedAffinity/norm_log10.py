...
def value_fit(
    val: np.ndarray, t: np.ndarray, tmax: int, eq: callable, delete_nan: bool = True
    ...
    for c in cases:
        if not c == "3.50_60":
            ...
            fits = fits.with_columns(
                ...
                pl.Series(value_fit(data, timestep, tmax=tmax, eq=erfc_exp)).alias(
                    f"{c}_erfc.exp"
                ),
            )
    return fits
...
uni = fitsm.filter(
    pl.col("distribution") == "uniform", pl.col("remaining").is_not_nan()
)
