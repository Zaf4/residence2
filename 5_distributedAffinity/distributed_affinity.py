...
def erfc(x, a, b, ks):
    return a * np.exp((np.square(b) * x) / ks) * erfc(np.sqrt((np.square(b) * x) / ks))
...
def value_fit(
    val: np.ndarray, eq: callable, sigma_w: bool = False
    ...
    if sigma_w:
        sigma = t**-10
        popt, pcov = curve_fit(eq, t, val, maxfev=20000000, sigma=sigma)
...
def scatterit_multi(
    ...
    **kwargs,
    ...
    # scattter plot (Data)------------------------------------------------------
    sns.scatterplot(
        data=df.sample(frac=0.25, random_state=42, weights=weights),
        x="timestep",
        y="value",
        palette=palette,
        hue="case",
...
if __name__ == "__main__":
    ...
    for i, eqx in enumerate([tri_exp, powerlaw, powerlaw]):
        ...
        for j, col_type in enumerate([uniform, gaus]):
            ...
            scatterit_multi(
                part[col_type], fits, i=j, j=i, axes=axes, palette=palettes[j]
            )
