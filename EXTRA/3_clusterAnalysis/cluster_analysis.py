...
def find_cluster(
    atoms: np.ndarray, threshold: float = 2.1, min_size: int = 5
) -> np.ndarray:
    """
...
def cluster_id(cluster_arr: np.ndarray, n: int = 0) -> np.ndarray:
    ...
    for j, c in enumerate(cluster_arr):
        ...
        index_in_df = (
            n + cluster_atoms
        )  # indexes in the df (n, number of not-to-be-clustered atoms)
...
def cluster_single_frame(
    frame: np.ndarray, min_size: int = 12, clusterit: bool = True
) -> pd.DataFrame:
    """
...
def frame_clusters2current(arr_name: str, skip: int = 1) -> None:
    """
    ...
    if not os.path.exists("./images2current"):
        os.mkdir("images2current")

    matplotlib.use("agg")
    for i, frame in track(
        enumerate(frames), total=len(frames), description="Processing..."
    ):
        ss = cluster_single_frame(frame)
        ax = imagify(ss)
        plt.savefig(f"./images2current/{i}.pbg", dpi=100)
        plt.close()
    plt.close("all")
...
def frame_clusters2index(arr_name: str, index: int = -1, skip: int = 1) -> None:
    """
    ...
    """
    frames = np.load(arr_name)[::skip]
    cluster_index = cluster_single_frame(frames[index]).clusterID

    if not os.path.exists("./images2index"):
        os.mkdir("images2index")
    ...
    for i, frame in track(
        ...
        ss["clusterID"] = cluster_index
        ax = imagify(ss)
        plt.savefig(f"./images2index/{i}.png", dpi=100)
        plt.close()

    return


def images2gif(gif_name: str = "clusters", path: os.PathLike = "./images") -> None:
    """
    ...
    """
    names_len = len(glob.glob(f"{path}/*.png"))
    names = [f"{path}/{i}.png" for i in range(names_len)]
    with imageio.get_writer(f"{path}/{gif_name}.gif", mode="I") as movie:
        for name in names:
...
def images2movie(movie_name: str = "clusters", path: os.PathLike = "./images") -> None:
    """
    ...
    if not os.path.exists("./movies"):
        os.mkdir("./movies")
    clip.write_videofile(f"./movies/{movie_name}.mp4", fps=24)

    return


def generate_cluster_info_table(
    frame: np.ndarray, timestep: int = 0, simple: bool = True
) -> pd.DataFrame:
    """
    ...
    for index in range(1, n_cluster + 1):
        ...
        rgM = [[rgxx, rgxy, rgxz], [rgxy, rgyy, rgyz], [rgxz, rgyz, rgzz]]
...
def labeledGraph(
    arr_name: str, index: int = -1, show_index: bool = False, savepng: bool = False
) -> plt.Axes:
    """
...
def mark_cluster_surface(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(int(num_clusters)):
        ...
        cl = cluster_frame[
            (cluster_frame.clusterID == i + 1) & (cluster_frame.type == 5)
        ]
        ids = np.array(cl["id"])
