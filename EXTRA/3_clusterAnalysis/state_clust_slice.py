...
def find_cluster(
    ...
    for i in range(num):
        distance = np.linalg.norm(
            np.subtract(atoms, atoms[i]), axis=1
...
def labeledGraph(arr_name: str, index: int = -1, savepng: bool = False) -> plt.Axes:
    ...
    for i in table.index:
        ...
        plt.text(x=table.x[i] + 3, y=table.y[i] + 3, s=label, backgroundcolor="red")
...
def main_work(dur: int = 20, sample: int = 20, save_loc: str = "csv"):
    ...
    for part in parts:
        type_slices = slice_clusters(
            fname="states.npz", ignore=900, duration=dur, slice_num=sample, part=part
        )
