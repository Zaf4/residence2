...
def bounds(arr: np.ndarray, trailing_zero: bool = True) -> np.ndarray:
    ...
    linear_distance = linear_distance.astype(
        np.int16
    )  # convert to i16 for tf_indeces are int
...
def bounds_dataframe(
    arr_L: np.ndarray, arr_R: np.ndarray, prefix: str = "case"
) -> pl.DataFrame:
    n_tf, n_timestep = arr_L.shape
    df = pl.DataFrame()

    # bound status
    bound_all = make_trailing_zeros(
        bounds(arr_L, trailing_zero=False) + bounds(arr_R, trailing_zero=False)
    )
...
def decay_plot(df: pl.DataFrame, until: int = 20):
    ...
    df_melted = df.head(until).melt(
        id_vars="timestep", value_name="counts", variable_name="case"
    )
